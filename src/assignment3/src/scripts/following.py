...
def callback(data):
	global y_l, y_r,x,f_d,front_scanrange,side_scanstartangle,side_scanrange
	...
	f_d= min(min(x[0:int(front_scanrange/2)],x[int(360-front_scanrange/2):360])) # front wall distance

def PID_controllerr():

	#Setup
	global k1,k2,k3,kp,kd,ki,f_d,x,y_r,y_l
	global fron_distance
	...
	rospy.init_node('PID_wallfollowing_control', anonymous=True)
	...
	while not rospy.is_shutdown():
		...
		delta = front_distance-y_r   # distance error 
		...
		#clip PID output
		angular_vel = np.clip(PID_output,-1.2,1.2)
		linear_vel   = np.clip((f_d-0.35),-0.1,0.4)

		
		
		print('distance from right wall in cm =',format(int(y_r*100)),'/',format(front_distance*100))
		print('distance from front wall in cm =',format(f_d*100))
		print('linear_vel=',format(linear_vel),' angular_vel=',format(angular_vel))
		rospy.loginfo('\n') 
